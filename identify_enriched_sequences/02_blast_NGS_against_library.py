...
def count_reads(library_sequences, r1_fastq, r2_fastq, min_identity = 0.90, min_length = 150, debug=0, outdir='.', test=0):
   ...
   if test:
      r1_seqs = r1_seqs[:test]
      r2_seqs = r2_seqs[:test]
...
def main():
   ...
   arg_parser.add_argument('-test', type=int, help="test -t N number of reads instead of full set) for debugging", default=0 )    
   args = arg_parser.parse_args()

   if not os.path.exists( 'matched_reads' ): os.mkdir('matched_reads')
   full_path = f'matched_reads/{args.subpool}_{args.tag}'
   ...
   count_df = count_reads(args.design_fasta, reads1, reads2, debug=1, outdir=full_path, test=args.test)
   print (count_df)
   print (args.design_fasta)
   design_pool_name_stem = args.design_fasta.replace('.fas','')
   print (design_pool_name_stem)
   design_pool_name_stem = design_pool_name_stem.split('/')[-1]
   print (design_pool_name_stem)
   tsv_name = f'matched_reads/{design_pool_name_stem}_{args.tag}_counts.tsv'
